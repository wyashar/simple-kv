...
const MAX_OP_LEN: usize = 8; // may change later, we only use 3 length now
const CRLF: &[u8; 2] = b"\r\n";

#[derive(Debug, thiserror::Error)]
pub enum KvRequestError {
    #[error("expected to find a CRLF when parsing after {0}")]
    MissingCrlf(String),
    ...
    KeyTooLarge(usize),
    #[error("operation length {0} exceeds maximum of {max}", max = MAX_OP_LEN)]
...
impl KvRequest {
    pub fn from_reader<T: BufRead>(reader: &mut T) -> Result<KvRequest, KvRequestError> {
        let mut request_len_bytes: [u8; 2] = [0; 2];
        reader.read_exact(&mut request_len_bytes)?;
        let request_len: u16 = u16::from_be_bytes(request_len_bytes);

        let mut payload: Vec<u8> = vec![0u8; request_len as usize];
        reader.read_exact(&mut payload)?;
        let mut cursor: &[u8] = payload.as_slice();

        Ok(Self {
            command: KvCommand::from_reader(&mut cursor)?,
        })
    }

    pub fn write_to<W: Write>(&self, w: &mut W) -> Result<(), std::io::Error> {
        let mut command_bytes: Vec<u8> = Vec::new();
        self.command.write_to(&mut command_bytes)?;

        let len: u16 = u16::try_from(command_bytes.len())
            .expect("request total byte length must be a valid u16");
...
impl KvCommand {
    ...
    fn key(&self) -> &[u8] {
        match self {
            Self::Del(k) | Self::Get(k) | Self::Put(k, _) => k,
        }
    }

    pub fn write_to<W: Write>(&self, w: &mut W) -> Result<(), std::io::Error> {
        w.write_all(self.name().len().to_string().as_bytes())?;
        w.write_all(CRLF)?;
        w.write_all(self.name().as_bytes())?;
        w.write_all(CRLF)?;
        w.write_all(self.key().len().to_string().as_bytes())?;
        w.write_all(CRLF)?;
        w.write_all(self.key())?;
        w.write_all(CRLF)?;

        if let Self::Put(_, val) = self {
            w.write_all(val.len().to_string().as_bytes())?;
            w.write_all(CRLF)?;
            w.write_all(val)?;
            w.write_all(CRLF)?;
        }
    ...
    pub fn from_reader<T: BufRead>(reader: &mut T) -> Result<KvCommand, KvRequestError> {
        let op_len = parse_len(reader, "operation length")?;
        ...
        let key_len = parse_len(reader, "key length")?;
        ...
        Ok(match op_str {
            ...
            "Del" => KvCommand::Del(key_bytes),
            "Put" => {
                let value_len = parse_len(reader, "value length")?;
...
fn consume_crlf<R: Read>(reader: &mut R, crlf_location: &str) -> Result<(), KvRequestError> {
    ...
    if &buf != CRLF {
        return Err(KvRequestError::MissingCrlf(crlf_location.to_owned()));
    }

    Ok(())
}

fn parse_len<R: BufRead>(reader: &mut R, crlf_location: &str) -> Result<usize, KvRequestError> {
    let mut buf: Vec<u8> = Vec::new();
    reader.read_until(b'\n', &mut buf)?;

    let bytes_trimmed = buf
        .strip_suffix(CRLF)
        .ok_or_else(|| KvRequestError::MissingCrlf(crlf_location.to_owned()))?;
...
impl fmt::Display for KvCommand {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if let Self::Put(_, val) = self {
            write!(f, " value={}", String::from_utf8_lossy(val))?;
        }
...
mod tests {
    ...
    fn from_reader_for_kv_command_bad_operator() {
        let mut byte_arr: &[u8] = b"7\r\nInvalid\r\n";
        let actual = KvCommand::from_reader(&mut byte_arr);
        assert!(matches!(actual, Err(KvRequestError::BadOperation(op)) if op == "Invalid"));
    }
