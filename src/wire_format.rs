...
impl Operation {
    ...
    pub fn execute(self, store: &mut KvStore) -> KvStoreResult {
        match self {
            Self::Put(key, value) => store.put(key, value),
            Self::Get(key) => store.get(&key),
            Self::Del(key) => store.del(&key),
        }
    }
}
...
impl fmt::Display for WireFormat {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Cmd(op) => write!(f, "op\r\n{op}"),
            Self::SimpleString(s) => write!(f, "sstr\r\n{s}\r\n"),
        }
    }
}
...
mod tests {
    ...
    fn wire_format_into_bytes_roundtrip_sstr() {
        let original = WireFormat::SimpleString("Hello World".to_string());
        let bytes = original.clone().into_bytes();
        let roundtripped = WireFormat::from_reader(&mut &bytes[..]).expect("valid bytes");
        assert_eq!(original, roundtripped);
    }
