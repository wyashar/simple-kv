...
pub enum KvResponseError {
    ...
    EmptyLine,
    #[error("unrecognized response prefix byte: {0:?}, expected one of {prefixes:?}", prefixes = [OKAY_PREFIX as char, ERROR_PREFIX as char, VALUE_PREFIX as char])]
    BadFirstChar(char),
...
impl<'a> KvResponse<'a> {
    ...
    pub fn from_reader<T: BufRead>(reader: &mut T) -> Result<KvResponse<'a>, KvResponseError> {
        ...
        match *prefix {
            OKAY_PREFIX => Ok(KvResponse::Okay),
            ERROR_PREFIX => {
                let msg = std::str::from_utf8(rest)?;
                Ok(KvResponse::Error(msg.to_owned()))
            }
            VALUE_PREFIX => Ok(KvResponse::Value(Cow::Owned(rest.to_owned()))),
