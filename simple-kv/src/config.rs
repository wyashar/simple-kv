...
const MIN_TTL_CLEANUP_INTERVAL_SECS: u64 = 30;
...
impl Config {
    pub fn from_env() -> Config {
        ...
        let ttl_cleanup_interval_secs: u64 = env::var(TTL_CLEANUP_INTERVAL)
            .unwrap_or_else(|_| panic!("{TTL_CLEANUP_INTERVAL} must be set"))
            .parse()
            .unwrap_or_else(|_| panic!("{TTL_CLEANUP_INTERVAL} must be a number of seconds"));
        if ttl_cleanup_interval_secs < MIN_TTL_CLEANUP_INTERVAL_SECS {
            panic!(
                "{TTL_CLEANUP_INTERVAL} must be at least {MIN_TTL_CLEANUP_INTERVAL_SECS} second"
            );
        }
        let ttl_cleanup_interval = Duration::from_secs(ttl_cleanup_interval_secs);
